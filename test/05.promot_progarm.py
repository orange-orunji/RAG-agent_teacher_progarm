...
# 获取客户端；环境变量 OPENAI_API_KEY 已配置时可省略 api_key 参数
...
# 定义文本
message = [
  {'role':"system",'content':"你是金融专家,将文本分类为['新闻报道','财务报道','公司公告','分析师报告'],不清楚的分类为'不清楚类型'"}
]

for key,value in examples_data.items():
  message.append({'role':"assistant",'content':f"案例名为:{key}"})
  message.append({'role':"assistant",'content':f"案例内容为:{value}"})
